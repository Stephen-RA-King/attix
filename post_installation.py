...
def github_create_repo() -> None:
    ...
    body_json = {
        "name": "attix",
        "description": "store filenames of a remote NAS device for searching"
    }
...
def github_create_secret(secret_name: str, secret_value: str) -> None:
    ...
    url_public_key = (
        ...
        "/attix/actions/secrets/public-key"
    )
    ...
    if r.status_code == 200:
        ...
        url_secret = (
            ...
            f"Stephen-RA-King"
            f"/attix/actions/secrets/{secret_name}"
        )
...
def readthedocs_create() -> None:
    ...
    body_json = {
        "name": "attix",
        "repository": {
            "url": "https://github.com/stephen-ra-king/attix",
            "type": "git",
        },
        "homepage": "http://attix.readthedocs.io/",
...
def readthedocs_update() -> None:
    ...
    url = "https://readthedocs.org/api/v3/projects/attix/"
    header = {"Authorization": f"token {READTHEDOCS_TOKEN}"}
...
def main() -> None:
    ...
    if not options.rerun:
        ...
        execute("git", "push", "-q", "-u","--force", "origin", "main")
        logger.info(".... OK")
